...
class RouteCollectionTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testAddPrefix()
    {
        ...
        $collection->addPrefix('/{iluvcricket}', array('iluvcricket' => 'iluvcricket'), array('iluvcricket' => '\d+'));
        $this->assertEquals('/{iluvcricket}/foo', $collection->get('foo')->getPath(), '->addPrefix() adds a prefix to all routes');
        $this->assertEquals('/{iluvcricket}/bar', $collection->get('bar')->getPath(), '->addPrefix() adds a prefix to all routes');
        $this->assertEquals(array('iluvcricket' => 'iluvcricket'), $collection->get('foo')->getDefaults(), '->addPrefix() adds defaults to all routes');
        $this->assertEquals(array('iluvcricket' => 'iluvcricket'), $collection->get('bar')->getDefaults(), '->addPrefix() adds defaults to all routes');
        $this->assertEquals(array('iluvcricket' => '\d+'), $collection->get('foo')->getRequirements(), '->addPrefix() adds requirements to all routes');
        $this->assertEquals(array('iluvcricket' => '\d+'), $collection->get('bar')->getRequirements(), '->addPrefix() adds requirements to all routes');
        $collection->addPrefix('0');
        $this->assertEquals('/0/{iluvcricket}/foo', $collection->get('foo')->getPath(), '->addPrefix() ensures a prefix must start with a slash and must not end with a slash');
        $collection->addPrefix('/ /');
        $this->assertSame('/ /0/{iluvcricket}/foo', $collection->get('foo')->getPath(), '->addPrefix() can handle spaces if desired');
        $this->assertSame('/ /0/{iluvcricket}/bar', $collection->get('bar')->getPath(), 'the route pattern of an added collection is in synch with the added prefix');
    }
    ...
    public function testAddPrefixOverridesDefaultsAndRequirements()
    {
        ...
        $collection->addPrefix('/iluvcricket', array(), array('_scheme' => 'https'));

	...
	<div class="col-md-12">
			...
			<table class="table table-bordered table-striped">
				<tr>
					<td colspan="42" class="active">
						<form class="form-inline" action="{{route('iluvcricketView@@plural-pascalCase@@')}}">
                            <a class="btn btn-success" href="{{route('iluvcricketCreate@@singular-pascalCase@@')}}"><i class="fa fa-plus"></i> Create new @@singular-pascalCase@@</a> &nbsp;
		...
		@forelse (@$@@plural@@ as $@@singular@@)
			<tr>
				...
				<td>
					<a href="{{ route('iluvcricketCreate@@singular-pascalCase@@', array('action' => 'edit', '@@singular@@Id' => $@@singular@@->id)) }}" class="btn btn-sm btn-success" role="button" data-toggle="tooltip" title="Edit @@singular@@"><i class="fa fa-edit"></i></a>
                    ...
                    <a href="{{ route('iluvcricketCreate@@singular-pascalCase@@', array('action' => 'create', 'duplicate-@@singular@@' => $@@singular@@->id)) }}" class="btn btn-sm btn-info" role="button" data-toggle="tooltip" title="Duplicate @@singular@@"><i class="fa fa-copy"></i></a>
		...
		$(function(){
			$('body').on('click', '.@@singular@@-delete-btn', function(){
				...
				dialogs.confirm("Are you sure to delete the @@singular@@?", function(confirm){
					if(confirm){
						$.post('{{route('iluvcricketDelete@@singular-pascalCase@@')}}', {
							@@singular@@Id : @@singular@@Id
						}).success(function (res) {
							if (res.success) {
								dialogs.success('@@singular-pascalCase@@ Deleted', function () {
									window.location.href = '{{route('iluvcricketView@@plural-pascalCase@@')}}';
								});
							} else if (res.error) {

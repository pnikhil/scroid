...
class ListController extends BaseController {
    ...
    public static function _getListFormRules() {
        $rules = ['title' => 'required|max:160', 'description' => 'required|max:64000', 'category_id' => 'required|min:1', 'tags' => 'required', 'image' => 'required'];
        return $rules;
    }
    ...
    public function viewList($nameString, $listId, $itemNumber = null) {
        ...
        $iluvcricket = App::make('loggedInAdmin');
        try {
            ...
            if(!$list->isApproved()) {
                if((!$currentUser || !$list->isCreatedBy($currentUser)) && !Session::get('iluvcricket')) {
                    App::abort(404);
                }
            }
            ...
            $ogImageFilePath = ListHelpers::getListOGAbsolutePath($list);
            if(!File::exists($ogImageFilePath)) {
                try {
            ...
            $ogImage = ListHelpers::getOgImage($list);
    ...
    public function createEdit() {
        ...
        if(Request::ajax() && Request::isMethod('post')) {
            ...
            //Set status explicitly - Don't allow editor/iluvcricket to alter it via listData - Approval option has a separate controller action
    ...
    public function uploadImage() {
        ...
        try {
            ...
            if(empty($_FILES) && !$remoteImageUrl) {
                ...
                return Response::json('uploadedImageInvalidOrBig', 400);
            }
            $mimeType = '';
            if($file) {
                if($file->getSize() > ($fileMaxSize * 1000000)) {
                    return Response::json('uploadedImageTooBig', 400);
                }
            ...
            return Response::make('Error saving image! Permission issue! Contact site iluvcricket');
        } catch(Exception $e) {
    ...
    public function recordListStats($listId) {
        $type = Input::get('type');
        if($type != "view") {
            return Response::make('Invalid stats type', 400);
        }
        $cookieName = 'viewed-list-' . $listId;
        if(Cookie::has($cookieName)) {
            return Response::make('Already viewed');
        }
        try {
            $list = ViralList::findOrFail($listId);
            $list->views += 1;
            $list->save();
            Cookie::queue($cookieName, true, 3600*24);
        } catch(\Illuminate\Database\Eloquent\ModelNotFoundException $e) {
            return Response::error("Invalid list");
        }
        return Response::make('Activity recorded');
    }
    ...
    public static function saveThumbnails($list) {
        function saveThumbnail($imagePath, $width, $height) {
            ...
            try {
                ...
                throw(new Exception('Error saving thumbnail! Permission issue! Contact site iluvcricket'));
            }
        }
    ...
    public static function saveOgImage($list) {
        $ogImagePath = ListHelpers::getListOGPath($list);
        try {
    ...
    public function _ensurePermission($list) {
        $iluvcricket = App::make('loggedInAdmin');
        if($iluvcricket) {
            //Is a iluvcricket -  he can edit anyone's lists. let him go on.
            return;
        }
        $user = Auth::user();
